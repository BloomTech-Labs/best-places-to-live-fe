...
export class MapContainer extends Component {
    ...
    getCities = async (top=true) => {
        ...
        let url = top ? 
                ...
                :`https://stagebe.letsmovehomie.com/city/location/?lat=${this.location.lat}&lng=${this.location.lng}&limit=200&zoom=${this.zoom}`;

        axios.post(url, getres)
        .then(
            (res) =>  {
                if(res.data.message) throw "no data";
                if(res.data.cities) res.data.data = res.data.cities;
                this.setState({...this.state, cities: res.data.cities });
                ...
                >
                    ...
                    this.state.cities ? this.state.cities.map(c => {
                        ...
                        return(
                            <Marker
                                key={c._id}
                                position={{ lat: parseFloat(c.location[1]), lng: parseFloat(c.location[0]) }}
                                name={c.name}
                                title={c.name}
                    ...
                    >
                        ...
                        <div>
                            {this.state.selectedPlace ? <TopTenCards card={this.state.selectedPlace}/> : ""}
                            {/* <h4>{this.state.selectedPlace ? this.state.selectedPlace.name : ""}</h4> */}
                        </div>
                    </InfoWindow>
                </Map>
                <div style={{position: "fixed", top: "100px", display: this.state.cities?"none":"flex", alignItems: "center"}}>
